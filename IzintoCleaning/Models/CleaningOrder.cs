...
namespace IzintoCleaning.Models
{
    public class CleaningOrder
    {
        ...
        [Required(ErrorMessage = "Enter the area of the house to be cleaned")]
        [Range(30, 200, ErrorMessage = "Size of house must be between 30 and 200 square meters")]
        public decimal area { get; set; }

        [Required(ErrorMessage = "Enter the number of floors to be cleaned")]
        [Range(1, 4, ErrorMessage = "We clean builidings with 1-4 floors")]
        ...
        {
            var cost = (from i in db.Cleanings where Package_ID == i.Package_ID select i.CostArea).SingleOrDefault();
            return cost;
        }

        public decimal getFloorCost()
        {
            var cost = (from i in db.Cleanings where Package_ID == i.Package_ID select i.CostStorey).SingleOrDefault();
            return cost;
        }
